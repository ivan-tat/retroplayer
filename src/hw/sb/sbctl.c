...
void PUBLIC_CODE _sb_set_hw_dsp(uint8_t type, uint16_t dspv)
{
    switch (type)
    {
    case 1:
...
void PUBLIC_CODE _sb_set_hw_flags(bool base, bool irq, bool dma8, bool dma16)
{
...
void PUBLIC_CODE _sb_set_hw_config(uint16_t base, uint8_t irq, uint8_t dma8, uint8_t dma16)
{
    sdev_hw_base = base;
    sdev_hw_irq = irq;
    sdev_hw_dma8 = dma8;
...
void PUBLIC_CODE _sb_set_mode(uint16_t rate, bool f_16bits, bool f_signed, bool f_stereo)
{
    sdev_mode_rate = rate;
...
bool PUBLIC_CODE _sb_detect_IRQ(uint8_t dma, bool f_16bits)
{
    uint16_t rate;
    bool f_stereo;
...
bool __near __pascal _check_value_type(uint16_t v)
{
    return v <= 6;
}

bool __near __pascal _check_value_dsp(uint16_t v)
{
    uint16_t i;
...
bool __near __pascal _check_value_irq(uint8_t v)
{
    uint16_t i;
...
bool __near __pascal _check_value_dma(uint8_t v)
{
    uint16_t i;
